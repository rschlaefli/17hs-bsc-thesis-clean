...
Our models are all based on the latest Keras 2.0 and Tensorflow 1.4 Python libraries, which have greatly simplified the creation of our models.
...
During the first 45 experiments, we have not applied any automated early stopping procedures. These procedures are often used to interrupt the training process once the performance stops improving (or once the difference between loss and validation loss increases again). Some experiments have, however, been interrupted manually, once it became clear that their performance would most certainly only get worse (specifically the experiments in EV09 and EV14).

\subsubsection{Final evaluation}
After we had found some model configurations that seemed to perform well and completed the primary tuning phase, we wanted to finally assess the performance of our models and reduce the variance in the test results. We thus performed a further round of evaluation where we retrained the best-performing models several times each and averaged their predictions. More specifically, we chose the five experiments that had the lowest validation loss (out of the experiments that used Split 3). These five final experiments belonged to the EV02, EV06, EV07 and EV12 evaluation schemes, whereof both experiments in EV02 were amongst the top five.

\paragraph{Evaluation procedure}
The final evaluations were performed as follows: first, the validation set was recombined with the training set, as it was no longer needed for tuning. This step allowed the final models to train on more data, potentially resulting in more representative results. The fixed validation set was instead replaced with a random 20\% holdout set during training (changing every epoch). The five final model configurations were then trained five times each. After the successful completion of all training rounds (consisting of 25 experiments), the predictions of repetitious trainings were averaged, yielding a final set of predictions. The accuracy of these predictions exhibits some interesting properties, as we will show in the remainder of this section.
...
The first model from EV02 (2-EV02) seems to perform best overall, with short-term accuracy of about 2.5 days MAE, mid-term accuracy of 3-3.5 days MAD and longer-term accuracy (up to one month before the onset) of around 2.5-3 days MAE. This is, by a fair margin, below the 4 days MAE that were described as the error of IMD predictions. Interestingly, this model is the only model that does not use additional convolutional layers and passes the inputs directly through ConvLSTM2D (after pooling), which might mean that these additional feature detectors are not as useful as they were thought to be.
...
When we combine the separate measures shown previously, we get a single accuracy measure over the entire test set. As shown in \cref{tab:overall_average}, \textit{2-EV02} is clearly the most accurate model, achieving an accuracy of around 2.8 days MAE. The other two models based on IMD onset dates follow the first model closely, while the models based on objective onset dates are significantly lagging behind. However, this might look different if objective onset dates were available for the entirety of the training and test years (i.e., 1979-2017).
...
\begin{table}[h!]
  ...
  \begin{tabular}{rc}
    ...
    \bottomrule
  \end{tabular}
  \caption{Overall averaged mean-absolute error for the five final evaluation models.}
