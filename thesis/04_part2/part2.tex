...
The onset of the Indian Summer Monsoon is a thoroughly-researched phenomenon. Many researchers have developed methods for the prediction of ISM onset and withdrawal dates that have been found to deliver relatively accurate predictions, even though the variability still poses a big challenge. The proposed approaches range from statistical analyses to, more recently, the application of machine learning methods and even deep learning. In \cref{sst:related_prediction}, we shortly introduce some of the proposed approaches (including the approach in \citet{Stolbova.2015}), focusing mainly on the achieved accuracy of predictions as a benchmark to our own predictions.

Many of our experimental models are based on a relatively new approach to training neural networks on spatiotemporal data. We specifically refer to the work of \citet{Shi.2015}, where a newly defined \textit{ConvLSTM} neural network layer was first applied to precipitation forecasting. Due to the reliance of many of our models on these layers, \cref{sst:neural_networks} is thought to provide intuition about their inner workings. We assume that the reader is familiar with neural network principles for the remainder of this work. However, as convolutional and recurrent layers are what ConvLSTM layers are based on, we explain some of their characteristics along the way.
